...
function main(){
    ...
    date_combo.innerHTML = "<option>Loading tracks...</option>"
    ...
    // Fill the combo 
    var filenames = list_all_files();
    
    // Link the date to the filenames in fileData
    filenames.forEach(function(filename){
        var req = new XMLHttpRequest();
        req.onreadystatechange = function() {
            if (this.readyState == 4 && this.status == 200) {
                var date = extractDate(this.responseText);
                if(fileData[date] == null){
                    fileData[date] = [];
                }
                // Add the filame to the date element in the JSON
                fileData[date].push(filename);
            }
        };
        req.open("GET", "../upload/files/"+filename, false);
        req.send();
    });
...
function list_all_files(){
    ...
    req.onreadystatechange = function() {
        if (this.readyState == 4 && this.status == 200) {
            ...
            var a_tags = xmlDoc.getElementsByTagName("a");
            for(var i=0 ; i<a_tags.length ; i++) {
                var filename = a_tags[i].getAttribute("href");
    ...
    req.open("GET", "../upload/files/", false);
    req.send();
    return output;
}
...
function renderClicked(){
    ...
    var initial_value = render_button.innerHTML;
    render_button.innerHTML="Rendering..."
    ...
    render_button.innerHTML=initial_value;
}
...
function addGpxFileToMap(gpxFile, id){
    ...
    req.open("GET", "../upload/files/"+gpxFile, false);
    req.send();
}
